...
{
    // Replica de Muestra Laboratorio (Modules/MuestraLaboratorio) para Faret: laboratorio
    // separado (equipo/analistas distintos a INNPACK, decision explicita del usuario), tablas
    // propias faret_muestra_laboratorio* (ver sql/2026_faret_laboratorio_modulo.sql), pero
    // reutiliza TAL CUAL los mismos DTOs/Request de QualityControlCenter.Modules.MuestraLaboratorio
    // (HumedadGuardarRequest, EnsayoDto, etc.) - son datos de ensayo genericos, sin nada
    // especifico de empresa, asi que duplicarlos no aportaria nada. Accion "faretLab" (no
    // "muestraLab") para no chocar con ese modulo INNPACK.
    public class FaretLaboratorioHandler
    {
        private readonly FaretLaboratorioRepository _repository;
        ...
        public FaretLaboratorioHandler(DbService db, CurrentUserSessionService session)
        {
            _repository = new FaretLaboratorioRepository(db);
            _session = session;
        }
        ...
        public async Task<string> Handle(string action, Dictionary<string, object> data)
        {
            try
            {
                ...
                {
                    var usuario = _session.GetCurrentUser();
                    var tipoEnsayo = action == "faretLab.rct.guardar" ? "RCT" : "FCT";
